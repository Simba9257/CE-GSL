...
from utils.reshape import get_cluster, reshape
        ...
        for run in range(runs):
            ...
            for i in range(iteration):
                model = SAGE(graph, in_feats, args.n_layers, n_hidden,
                             n_classes, dropout, args.dataset)
                ...
                if args.dataset in {'cora', 'citeseer', 'pubmed'}:
                    new_edge_index = reshape(logits, edge_index_2, weight, adj_matrix,
                                              args.k, args.dataset)
                    new_edge_index_2 = reshape(logits, edge_index_2, weight, adj_matrix,
                                                args.k, args.dataset)
