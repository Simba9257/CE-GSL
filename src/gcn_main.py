...
from utils.reshape import get_cluster, reshape2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--drop_rate', type=float)  # 0.5
    parser.add_argument('--n_hidden', type=int)  # 8 16 32 48 64
    parser.add_argument('--n_layers', type=int, default=1)  # 2 layers
    parser.add_argument('--runs', type=int, default=1)
    parser.add_argument('--iteration', type=int, default=10)
    parser.add_argument('--learning_rate', type=float)  # 0.01
    parser.add_argument('--weight_decay', type=float)  # 5e-4
    parser.add_argument('--num_epoch', type=int, default=200)
    parser.add_argument('--k', type=float, default=3)  # θ  0.5, 1, 3, 5, 10, 30
    ...
    for sp in splits:
        ...
        for run in range(runs):
            ...
            for i in range(iteration):
                ...
                logits = model(features)  # embbeding
                ...
                if args.dataset in {'cora', 'citeseer', 'pubmed'}:
                    new_edge_index = reshape2(logits, edge_index_2, weight, adj_matrix,
                                             args.k, args.dataset)
                    new_edge_index_2 = reshape2(logits, edge_index_2, weight, adj_matrix,
                                               args.k, args.dataset)
